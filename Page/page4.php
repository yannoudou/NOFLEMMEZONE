  ...
  if(isset( $_GET['id']) AND $_GET['id'] > 0)
   {
    ...
    $userdon=$ruser -> fetch();
     ...
     if(isset($_SESSION['id']) AND $userdon['id'] == $_SESSION['id']){
														       ...
														       if(isset($_POST['radio']) AND !empty($_POST['radio'])){
																		$sexo=$_POST['radio'];
																			 $insertsex = $bdd->prepare("UPDATE membres SET programme = ? WHERE id = ?");
														                           $insertsex->execute(array($sexo, $_SESSION['id']));
														                           $ruser3 = $bdd->prepare("SELECT ttaille,tcou,taille,thanche,sex FROM membres WHERE id=?");
                                                                    $ruser3->execute(array($userdon['id']));
                                                                    $userdon1=$ruser3 -> fetch();
                                                                  $userdon1['ttaille'];
                                                                   $userdon1['tcou'];
                                                                   $userdon1['thanche'];
                                                                  $userdon1['taille'];
                                                     ...
                                                     if ($userdon1['sex']== "man") {
                                                      $bodyfat=86.010*(log10( $userdon1['ttaille']-$userdon1['tcou']))-70.041*(log10( $userdon1['ttaille']));
                                                       $insertsex1 = $bdd->prepare("UPDATE membres SET bodyfat = ? WHERE id = ?");
                                                       $insertsex1->execute(array($bodyfat, $_SESSION['id']));
                                                       header('Location: page5.php?id='.$_SESSION['id']);
                                                       
                                                     }
                                                     else{

                                                      $bodyfat=163.205*(log10( $userdon1['ttaille']+ $userdon1['thanche']-$userdon1['tcou']))-97.684*(log10( $userdon1['ttaille']));
                                                       $insertsex1 = $bdd->prepare("UPDATE membres SET bodyfat = ? WHERE id = ?");
                                                       $insertsex1->execute(array($bodyfat, $_SESSION['id']));
                                                       header('Location: page5.php?id='.$_SESSION['id']);
                                                     }
