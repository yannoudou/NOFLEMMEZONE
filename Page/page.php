								   ...
								   if( !empty ($emailcon) AND !empty($passcon) AND !empty( $_POST['emailcon']) AND !empty( $_POST['passcon']))
									  {
												   ...
												   if($membreexiste == 1)
												   {
																			  $ruser1= $bdd->prepare("SELECT * FROM membres WHERE pseudo=? OR email=?  AND motdepasse= ? AND confirme= 1 ");
																			 $ruser1->execute(array($emailcon,$emailcon, $passcon));
  ...
  <form role="form"action="" method="POST" style="padding-left:120px ;">
	...
	<div class="caption">
	 <form role="form"action="" method="POST">
		<a href=""><input type="radio" class="" name="gender"<?php if (isset($gender) && $gender=="female")?> value="woman" style="height: 220px;width: 220px;margin-left: -5px; margin-top:-5px; "></a>
	</form>
	</div>
</div>
    </div>
